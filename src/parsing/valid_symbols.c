...
/*   Updated: 2024/07/25 23:25:38 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			if (++(*player_count) > 1)
// 			{
// 				error_exit_map("The player's position symbol is not"
// 					"correct the number is incorrect", map);
// 			}
...
void	check_map_row(t_map *map, const char *row, int *player_count)
{
	...
	while (row[col])
	{
		if (valid_symbol_character(row[col]) || row[col] == ' ')
		{
			if (++(*player_count) > 1)
